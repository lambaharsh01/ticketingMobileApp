...
useEffect(async()=>{
  let data=await getArray('busColor');
  if(data){
    setbusColor(data)
  }
},[])
...
const [busColor, setbusColor]=useState(['#2E81EB', '#E48B40', '#3FA1AD', '#219652'])

return(
<View style={{flex:1}}>
<StatusBar backgroundColor='#00adb0' />
<ScrollView>
<Text style={styles.validationHeaderText}>Bus Color Info</Text> 
...
<Pressable onPress={()=>{saveArray('busColor', busColor)}}  style={styles.saveButton} ><Text style={[styles.buttonText, styles.saveButtonText]}>Save</Text></Pressable> 
