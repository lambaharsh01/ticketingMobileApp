...
useEffect(async()=>{
  let data=await getArray('busStops');
  if(data){
    setBusStops(data)
  }
},[])
...
<ScrollView>
<Text style={styles.validationHeaderText}>Bus Stop Info</Text> 
...
<Pressable onPress={()=>{saveArray('busStops', busStops)}}  style={styles.saveButton} ><Text style={[styles.buttonText, styles.saveButtonText]}>Save</Text></Pressable> 
