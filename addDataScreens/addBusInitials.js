import {View, ScrollView, StatusBar, TextInput, Text, StyleSheet, Pressable, Modal, Alert } from 'react-native';
...
useEffect(async()=>{
let data=await getArray('busInitials');
if(data){    
    setBusInitialCode(data);
}
},[])
...
const [busInititalCode, setBusInitialCode]=useState(['DL1PC', 'DL1PD'])

return(
<View style={{flex:1}}>
<StatusBar backgroundColor='#00adb0' />
<ScrollView>
<Text style={styles.validationHeaderText}>Bus Initial/Starting Codes</Text> 
<Text style={styles.buttonAlignment}>


<Ionicons name="add-circle" size={38} style={[styles.buttonText, styles.fontAsthetics]} onPress={()=>{
  setBusInitialCode(prev=>{
    return [...prev, ''];
  })
}} />
</Text>
...
<Pressable onPress={()=>{saveArray('busInitials', busInititalCode)}}  style={styles.saveButton} ><Text style={[styles.buttonText, styles.saveButtonText]}>Save</Text></Pressable> 
