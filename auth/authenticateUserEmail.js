...
export default function App() {
  ...
  function backAction() {
    Alert.alert(
      "Are you sure?",
      "All the changes will be lost if you exit now.",
      [
  ...
  function sendOtp() {
    ...
    axios
      .post(baseUri + "/api/user/userEmailVerification", {
        userName,
        userEmail,
      })
      .then((res) => {
  ...
  function checkOtp() {
    ...
    axios
      .post(baseUri + "/api/user/varifyEmail", { otp: otpEntered, userEmail })
      .then((res) => {
        setProcess((prevCount) => prevCount + 1);
        setIsLoading(false);
        setOtpEntered("");
  ...
  function saveBasicDetails() {
    ...
    axios
      .put(baseUri + "/api/user/addUserDetails/" + userEmail, {
        gender,
        district,
      ...
      .then((res) => {
        if (!res?.data?.success ?? null)
          return Alert.alert(
            ...
            !res?.data?.message ?? "Please try again after some time"
          );
      ...
      .catch((err) => {
        ...
        Alert.alert(
          "Something Went Wrong",
          "Please try again after some time" + err
        );
        reloadStates();
      });
  }

  async function saveEmailAndProceed() {
    await AsyncStorage.setItem("userEmail", userEmail);
    await AsyncStorage.setItem("levelOfAuthorisation", "1");
    ...
    <View style={{ flex: 1, alignItems: "center" }}>
      ...
      {!isLoading && (
        <>
          ...
          {process === 0 && (
            <>
              <Text style={[styles.inputHeading, { marginTop: "11%" }]}>
                Enter Your Name
              </Text>
              ...
              <Text style={[styles.inputHeading, { marginTop: "17%" }]}>
                Enter Your Email ID
              </Text>
              <View style={styles.inputView}>
                <TextInput
