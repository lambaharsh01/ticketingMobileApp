  ...
  .then(async(res)=>{
    if(res.data.success){
      if(res.data.data.validUntil && res.data.data.ticketUploadInterval){
        ...
        Alert.alert('Admin Authentication Success', `You can generate tickets till ${res.data.data.validUntil.split('-').reverse().join('-')} and you wil be required to upload your ticket data with the generation of every ${res.data.data.ticketUploadInterval} tickets`,[
